...
def PadMask(padded_input, input_lengths):
    ...
    # Create position indices for each sequence element
    positions = torch.arange(T, device=padded_input.device).expand(N, T)
    
    # Compare positions with sequence lengths to create the mask
    # True where position >= length (padding), False otherwise
...
def CausalMask(padded_input):
    ...
    # diagonal=1 starts the 1s one position above the main diagonal
