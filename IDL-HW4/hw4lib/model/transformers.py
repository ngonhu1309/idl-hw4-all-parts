...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.positional_encoding    = PositionalEncoding(d_model, max_len=max_len) # Positional encoding
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        max_len = x_enc.size(1)
        pad_mask_src = torch.arange(max_len, device=x_enc.device).expand(len(x_enc_lengths), max_len) >= x_enc_lengths.unsqueeze(1)
    ...
    def decode(
        ...
        B, tgt_len = padded_targets.shape

        # TODO: Create target padding mask on the same device as the input
        pad_mask_tgt = None
        if target_lengths is not None:
            pad_mask_tgt = torch.arange(tgt_len, device=padded_targets.device).expand(B, tgt_len) >= target_lengths.unsqueeze(1)
        ...
        causal_mask = torch.triu(torch.ones(tgt_len, tgt_len, device=padded_targets.device), diagonal=1).bool()
        
        # TODO: Apply the embedding, positional encoding, and dropout
        x_dec = self.target_embedding(padded_targets) * (self.d_model ** 0.5)
