...
class ASRDataset(Dataset):
    def __init__(
        ...
        if self.partition != "test-clean":
            ...
            fbank_basenames = [os.path.basename(f).lower() for f in self.fbank_files]
            text_basenames = [os.path.basename(f).lower() for f in self.text_files]
            
            # Verify exact filename matches
            if fbank_basenames != text_basenames:
                # Find the first mismatch for better error reporting
                for i, (fb, tx) in enumerate(zip(fbank_basenames, text_basenames)):
                    if fb != tx:
                        print(f"Debug: FBANK '{fb}' vs TEXT '{tx}'")
                        raise ValueError(f"Misaligned files at position {i}: FBANK '{fb}' vs TEXT '{tx}'")
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                transcript_arr = np.load(self.text_files[i])
                transcript = ''.join(list(transcript_arr))
                ...
                self.transcripts_shifted.append(torch.LongTensor(shifted))
                self.transcripts_golden.append(torch.LongTensor(golden))
