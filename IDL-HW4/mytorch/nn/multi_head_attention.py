...
class MultiHeadAttention:
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None):
        ...
        q = q.reshape(self.N, self.L, self.num_heads, head_dim).transpose(0, 2, 1, 3)
        # (N, S, embed_dim) -> (N, num_heads, S, embed_dim // num_heads)
        k = k.reshape(self.N, self.S, self.num_heads, head_dim).transpose(0, 2, 1, 3)
        # (N, S, embed_dim) -> (N, num_heads, S, embed_dim // num_heads)
        v = v.reshape(self.N, self.S, self.num_heads, head_dim).transpose(0, 2, 1, 3)
        ...
        if key_padding_mask is not None or attn_mask is not None:
            mask = np.zeros((self.N, self.num_heads, self.L, self.S), dtype=bool)
            
            if key_padding_mask is not None:
                # Expand key_padding_mask to (N, 1, 1, S)
                expanded_key_mask = key_padding_mask[:, np.newaxis, np.newaxis, :]
                # Broadcast to (N, H, L, S)
                mask = mask | np.broadcast_to(expanded_key_mask, (self.N, self.num_heads, self.L, self.S))
                
            if attn_mask is not None:
                # Expand attn_mask to (1, 1, L, S)
                expanded_attn_mask = attn_mask[np.newaxis, np.newaxis, :, :]
                # Broadcast to (N, H, L, S)
                mask = mask | np.broadcast_to(expanded_attn_mask, (self.N, self.num_heads, self.L, self.S))
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        key_mask = key_padding_mask[:, np.newaxis, np.newaxis, :].repeat(self.num_heads, axis=1).repeat(self.L, axis=2)
        
        # Expand attn_mask to (1, 1, L, S) and broadcast to (N, H, L, S)
        attention_mask = attn_mask[np.newaxis, np.newaxis, :, :].repeat(self.N, axis=0).repeat(self.num_heads, axis=1)
    ...
    def _split_heads(self, x):
        ...
        x = x.reshape(self.N, -1, self.num_heads, self.embed_dim // self.num_heads)
